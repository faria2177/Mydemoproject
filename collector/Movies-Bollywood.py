...
class M3UCollector:
    ...
    def filter_active_channels(self):
        ...
        if not all_ch:
            logging.warning("No channels to filter!")
            return
        logging.info(f"Checking {len(all_ch)} links for activity...")
        seen = set()
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                try:
                    ...
                    logging.error(f"Error checking {ch.get('url', 'unknown')}: {e}")
        self.channels = active
        logging.info(f"Active links found: {sum(len(v) for v in active.values())}")
    ...
    def process_sources(self, source_urls):
        ...
        for url in source_urls:
            ...
            if content and url.endswith('.html'):
...
def main():
    ...
    if collector.channels:
        ...
        logging.info(f"Done! Total Bollywood movies collected: {total}")
    else:
        logging.error("No channels collected - exiting without commit")
        exit(1)
